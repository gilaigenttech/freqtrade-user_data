...
class PyTorchLSTMModel(nn.Module):
    ...
    :param input_dim: The number of input features. This parameter specifies the number of features in the input data that the LSTM will use to make predictions.
    :param output_dim: The number of output classes. This parameter specifies the number of classes that the LSTM will predict.
    :param hidden_dim: The number of hidden units in each LSTM layer. This parameter controls the complexity of the LSTM and determines how many nonlinear relationships the LSTM can represent. Increasing the number of hidden units can increase the capacity of the LSTM to model complex patterns, but it also increases the risk of overfitting the training data. Default: 100
    :param dropout_percent: The dropout rate for regularization. This parameter specifies the probability of dropping out a neuron during training to prevent overfitting. The dropout rate should be tuned carefully to balance between underfitting and overfitting. Default: 0.3
    :param n_layer: The number of LSTM layers. This parameter specifies the number of LSTM layers in the model. Adding more layers can increase its capacity to model complex patterns, but it also increases the risk of overfitting the training data. Default: 1
